...
using TLAuto.Device.PLC.View.Models.Events;
...
namespace TLAuto.Device.PLC.View.Models
{
    public class PLCDeviceInfo : ViewModelBase
    {
        ...
        private readonly ConcurrentDictionary<int, SwitchStatus> _cacheSwitchs = new ConcurrentDictionary<int, SwitchStatus>();
        internal readonly LogWraper LogWraper = new LogWraper(LogModuleName);
        private AutoUploadSwitchStatusExCommand _autoUploadCommand;
        ...
        private void AnalyseDatas()
        {
            while (!_isWhileAnalyseData)
            {
                ...
                foreach (var data in caches)
                {
                    ...
                    if (_autoUploadCommand.ParseReceivedData(data, out switchItems))
                    {
                        foreach (var switchItem in switchItems)
                        {
                            AddSwitchToCache(switchItem.SwitchNumber, switchItem.SwitchStatus);
                        }
                    }
                }
            }
        ...
        public override void Cleanup()
        {
            ...
            Parent.TLSerialPort.DataReceived -= TLSerialPort_DataReceived;
            base.Cleanup();
        }

        private void AddSwitchToCache(int swichNumber, SwitchStatus switchStatus)
        {
            var switchNumberKey = swichNumber - 1;
            if (_cacheSwitchs.ContainsKey(switchNumberKey))
            {
                SwitchStatus removeSwitchStatus;
                _cacheSwitchs.TryRemove(switchNumberKey, out removeSwitchStatus);
                _cacheSwitchs.TryAdd(switchNumberKey, switchStatus);
                if (removeSwitchStatus != switchStatus)
                {
                    OnSwitchItemChanged(new SwitchItemChangedEventArgs(new SwitchItem(swichNumber, switchStatus)));
                }
            }
        }
        ...
        public async Task<byte[]> ControlPLC(PLCControlServiceData serviceData)
        {
            ...
            switch (serviceData.ControlPLCType)
            {
                ...
                {
                    var switchStatusList = _cacheSwitchs.Values;
                    var index = 1;
                    var switchList = switchStatusList.Select(switchStatus => new SwitchItem(index++, switchStatus)).ToList();
                    return switchList.ToBytes();
        ...
        private void ResetControlItem()
        {
            ...
            for (var i = 0; i < DigitalSwitchNumber; i++)
            {
                ...
                _cacheSwitchs.TryAdd(i, SwitchStatus.NO);
            }
            RelayInfos.Clear();
            ...
            _autoUploadCommand = new AutoUploadSwitchStatusExCommand(DeviceNumber, DigitalSwitchNumber);
        }

        internal void OpenedPort()
        {
            if (Parent != null)
            {
                Parent.TLSerialPort.DataReceived -= TLSerialPort_DataReceived;
        ...
        #region Event
        public event EventHandler<SwitchItemChangedEventArgs> SwitchItemChanged;

        protected virtual void OnSwitchItemChanged(SwitchItemChangedEventArgs e)
        {
            SwitchItemChanged?.Invoke(this, e);
        }
        #endregion
    }
}